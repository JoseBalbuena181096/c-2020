...
/*
        Source[0]=Point2f(220,120);
        Source[1]=Point2f(420,120);
        Source[2]=Point2f(0,305);
        Source[3]=Point2f(640,305);
        //init points to desination
        Destination[0]=Point2f(210,0);
        Destination[1]=Point2f(430,0);
        Destination[2]=Point2f(210,360);
        Destination[3]=Point2f(430,360);
...
/*
 ...
 int Loc_Max(Mat &img,int win_x_low,int win_x_high,int  win_y_low,int win_y_high)
        {
            //Create histogram with the length of the width of the frame 
	        vector<int> histogramLane;
            int Position = 0;
	        ...
	        Mat frame;  
            img.copyTo(frame);
		    frame = frame(Rect(win_x_low,win_y_low,(win_x_high-win_x_low),(win_y_high-win_y_low)));
            init_row = 1;	
		    end_row = frame.rows-1;
            ...
            Ptr = max_element(histogramLane.begin(),histogramLane.end());
            Position = distance(histogramLane.begin(),Ptr);
	        return  Position;
        }
		...
		//circle(out_img,cv::Point(rightx_current,1),cvRound((double)4/ 2), Scalar(255, 255, 0), 2);
		//circle(out_img,cv::Point(leftx_current ,1),cvRound((double)4/ 2), Scalar(255, 255, 0), 2);
		    ...
		    {
                //Loc_Max(img,win_xleft_low,win_xleft_high,win_y_low,win_y_high);
	    	     mean_leftx /=  count_left;
 //leftx_current = mean_leftx;
			    leftx_current = win_xleft_low + Loc_Max(img,win_xleft_low,win_xleft_high,win_y_low,win_y_high);
   	        }
	        if(count_right>=minpix)
		    {
	    	    mean_rightx /=  count_right;
//rightx_current =  mean_rightx ;
			    rightx_current =  win_xright_low + Loc_Max(img,win_xright_low,win_xright_high,win_y_low,win_y_high);
   	        }
		//circle(out_img,cv::Point(rightx_current,win_y_high),cvRound((double)4/ 2), Scalar(0, 255, 0), 2);
		//circle(out_img,cv::Point(leftx_current ,win_y_high),cvRound((double)4/ 2), Scalar(0, 255, 0), 2);
