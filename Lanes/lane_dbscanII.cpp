    ...
    */
    public:
        ...
        LineDetection():it_(nh_)
        {
            ...
            */
            Source[0]=Point2f(210*0.6,180*0.6);
            Source[1]=Point2f(430*0.6,180*0.6);
            Source[2]=Point2f(40*0.6,360*0.6);
            Source[3]=Point2f(600*0.6,360*0.6);
            //init points to desination
            Destination[0]=Point2f(170*0.6,0);
            Destination[1]=Point2f(470*0.6,0);
            Destination[2]=Point2f(170*0.6,360*0.6);
            Destination[3]=Point2f(478*0.6,360*0.6); 
	   	    distance_center = 0.0;
	        center_cam = 0;
            center_lines = 0;
		    angle = 0;
...
*/
	        Perspective(frame);
		frame = frame(Rect(68,1,frame.cols-136,frame.rows-1));
        ...
        Mat Threshold(Mat frame)
        {
	        ...
	        cvtColor(frame,frameHSV, COLOR_BGR2HSV);	 
  	        inRange(frameHSV, Scalar(0,0,140), Scalar(179,255,255),frameHSV);
	        cvtColor(frame,frameGray, COLOR_BGR2GRAY);
	        inRange(frameGray,120,255,frameGray);
	        bitwise_or(frameGray,frameHSV,frameThreshold);
	        erode(frameThreshold,frameThreshold, getStructuringElement(MORPH_RECT, Size(5, 5)));
            dilate(frameThreshold,frameThreshold, getStructuringElement(MORPH_RECT, Size(5,5)));
	        width_filter(frameThreshold,18);
	    ...
	    vector<vector<PointC>> data_cluster;
            int MINIMUM_POINTS = 10.0;// minimum number of cluster
            int EPSILON = 5;// distance for clustering
            data = non_zeros(img);
            DBCAN dbScan(52,EPSILON,MINIMUM_POINTS,data);
            dbScan.run();
		...
		for(int i=0;i<data_cluster.size()-1;++i)
		{
		   ...
		   {
		   	if(data_cluster[i].back().getDis(data_cluster[j].front())<(128*128))
			{
		...
		else
		{
			...
			for(int j=0;j<i.size();++j)
			{
			   ...
			   if(row_max[j] >= 0  && i[j] >=0 && dist_>146 && dist_<206)
				row_min[j] = i[j];
			}	
		     }	
		     for(int i=0;i<row_max.size();++i)
