    ...
    public:
        //Constructor
         LineDetection():it_(nh_){
            ...
            Source[0]=Point2f(600*0.25,720*0.25);
            Source[1]=Point2f(1964*0.25,720*0.25);
            Source[2]=Point2f(0,1240*0.25);
            Source[3]=Point2f(2464*0.25,1240*0.25);
            //init points to desination
            Destination[0]=Point2f(0,0);
            Destination[1]=Point2f(2464*0.25,0);
            Destination[2]=Point2f(0,1440*0.25);
            Destination[3]=Point2f(2464*0.25,1440*0.25);  
        }
        //Destructor
        ~LineDetection(){
	    ...
	    */
        ...
        Mat Threshold(Mat frame){
            ...
            //cv::filter2D(frameThreshold, output, -1, kernel, anchor, 0, cv::BORDER_DEFAULT);
            return output;
        }
        ...
        Mat plotLanes(Mat inputImage,vector<Point> lane) {
            ...
            if(left_flag&&right_flag){
                ...
                //Right line
                line(output, lane[0], lane[1], Scalar(0, 255, 255), 8, CV_AA); 
                //Left line
                line(output, lane[2], lane[3], Scalar(0, 255, 255), 8, CV_AA);
                x_offset = static_cast<double>((lane[3].x+lane[1].x)/2-(output.cols/2));
                y_offset = static_cast<double>(output.rows - 1);
		        fillConvexPoly(inputImage, poly_points, Scalar(255, 0, 0), CV_AA, 0);
