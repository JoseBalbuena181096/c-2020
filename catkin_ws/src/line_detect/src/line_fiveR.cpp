...
class LineDetection{
	...
	angle_line_pub = nh_.advertise<std_msgs::UInt8>("/angle_line_now",1000);
        ...
        Source[0]=Point2f(874*0.15,620*0.15);
        Source[1]=Point2f(1590*0.15,620*0.15);
        Source[2]=Point2f(14*0.15,1200*0.15);
        Source[3]=Point2f(2450*0.15,1200*0.15);
        //init points to desination
        Destination[0]=Point2f(884*0.15,0);
        Destination[1]=Point2f(1580*0.15,0);
        Destination[2]=Point2f(884*0.15,1440*0.15);
        Destination[3]=Point2f(1580*0.15,1440*0.15);  
	    distance_center = 0.0;
	    center_cam = 0;
        center_lines = 0;
		angle = 0;
	    ...
	    int *Histogram(Mat &img){
	        ...
	        Mat ROILane;
	        Mat frame;  
		    init_row = (img.rows*3/4);	
		    end_row = (img.rows/4)-1;
