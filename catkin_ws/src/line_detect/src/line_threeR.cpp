...
class LineDetection{
    ...
    public:
        ...
        Source[0]=Point2f(754*0.15,695*0.15);
        Source[1]=Point2f(1710*0.15,695*0.15);
        Source[2]=Point2f(14*0.15,1200*0.15);
        Source[3]=Point2f(2450*0.15,1200*0.15);
        //init points to desination
        Destination[0]=Point2f(764*0.15,0);
        Destination[1]=Point2f(1700*0.15,0);
        Destination[2]=Point2f(764*0.15,1440*0.15);
        Destination[3]=Point2f(1700*0.15,1440*0.15); 
	    distance_center = 0.0;
	    center_cam = 0;
        center_lines = 0;
		angle = 0;
	    ...
	    int *Histogram(Mat &img){
			...
			if(white_right>white_left){
				for(int r=0;r<img.rows;r++){
					...
					if(black_to_white && (count_white>=22)){
			   			c = c-23;
			   			big_white = true;
					}
	...
	void locate_lanes(Mat &img,Mat &out_img){
	    ...
	    for(int window=0;window<nwindows;window++){
	        ...
	        for(int r = win_y_low;r<win_y_high;r++){
                ...
                max_right = img.at<uchar>(r,win_xright_low);
				isMaxLeft = false;
				isMaxRight = false;
