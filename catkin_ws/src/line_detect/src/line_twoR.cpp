...
class LineDetection{
	...
	angle_line_pub = nh_.advertise<std_msgs::UInt8>("/angle_line_now",1000);
        ...
        Source[0]=Point2f(784*0.15,676*0.15);
        Source[1]=Point2f(1680*0.15,676*0.15);
        Source[2]=Point2f(0*0.15,1200*0.15);
        Source[3]=Point2f(2464*0.15,1200*0.15);
        //init points to desination
        Destination[0]=Point2f(784*0.15,0);
        Destination[1]=Point2f(1680*0.15,0);
        Destination[2]=Point2f(784*0.15,1440*0.15);
        Destination[3]=Point2f(1680*0.15,1440*0.15);  
	   distance_center = 0.0;
	    center_cam = 0;
        center_lines = 0;
		angle = 0;
		...
		uchar *pixel_h = img.ptr<uchar>(img.rows*3/4);
			...
			if(white_right>white_left){
				...
				uchar *pixel = img.ptr<uchar>(r);
					...
					if(black_to_white && (count_white>=16)){
			   			c = c-17;
			   			big_white = true;
					}
